...
const Poll = ({ poll }) => {
  ...
  const userVoted = poll.votes.findIndex(
    (id) => id === user?.result?._id || id === user?.result?.googleId
  );
  ...
  return (
    <Card className={classes.mainCard}>
      <CardContent className={classes.headerCard}>
        ...
        <Typography variant="body2">{`Expires on: ${DTexpire.toLocaleString(
      ...
      <CardActions className={classes.actionsCard}>
        ...
        {(user?.result?.googleId === poll.author ||
          user?.result?._id === poll.author) && (
        ...
        <CardContent className={classes.btnGroup}>
          ...
          <ButtonGroup disabled={isExpired} orientation="vertical" variant="outlined" size="small">
            {poll.choices.map((choice) => (
              <Button
                key={choice._id}
                onClick={() => dispatch(votePoll(poll._id, choice._id))}
              >
                {choice.text} &nbsp; {userVoted !== -1 && choice.votes.length}
              </Button>
            ))}
          </ButtonGroup>
        </CardContent>
